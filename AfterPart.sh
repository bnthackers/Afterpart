...
if [ "$ArCh" = "i386" ] || [ "$ArCh" = "i686" ]; then
  ...
  dEd="x64"
  arch="wine64"
  PgFi="Program Files" # default value
...
RhI="/home/garvit/hackathon/ImgBackdoor/bin/rhkr.exe"
...
!
  ...
  sleep 2
    ...
    if [ "$NoMsF" = "YES" ]; then
      ...
      sleep 2
    echo ""
    echo ${white}    Codename${RedF}::AfterPart;
    echo ${white}    Author${RedF}::${white}$ken ${RedF}::${white}[Garvit Sharma, Shivam Kumar, Gaurav Anjna]${Reset};
    sleep 1
    service apache2 stop | zenity --progress --pulsate --title "PLEASE WAIT" --text="Stop apache2 webserver" --percentage=0 --auto-close --width 300 > /dev/null 2>&1
    if ! [ "$NoMsF" = "YES" ]; then
    service postgresql stop | zenity --progress --pulsate --title "PLEASE WAIT" --text="Stop postgresql service" --percentage=0 --auto-close --width 300 > /dev/null 2>&1
